#!/usr/bin/env python3
"""
Crypto Bot Pro v36.0.0.0 - Sistema Ultra-Preciso de Trading con IA
Aplicacion de escritorio usando PyQt5
TRIPLE ESTRATEGIA COMBINADA PARA 85% DE RENTABILIDAD CONSISTENTE
Desarrollado por Lic. Adolfo Daniel Aguirre

MEJORAS v36.0.0.0 (Sistema Triple Estrategia - 85% Win Rate):
   ============================================================
   ESTRATEGIA 1: MOMENTUM INSTITUCIONAL (35% peso)
   - Detecta flujo de dinero institucional via volumen
   - Analiza Order Flow con delta de volumen
   - Confirma con VWAP y desviaciones estandar
   - Filtra con divergencias ocultas RSI/MACD
   
   ESTRATEGIA 2: ESTRUCTURA DE MERCADO (35% peso)  
   - Analiza quiebres de estructura (BOS/CHoCH)
   - Detecta zonas de liquidez institucional
   - Identifica FVG (Fair Value Gaps) para entradas
   - Confirma con Order Blocks y Breaker Blocks
   
   ESTRATEGIA 3: CONFLUENCE MULTI-TIMEFRAME (30% peso)
   - Alineacion perfecta 5m/15m/30m/1h
   - Fibonacci dinamico con extensiones
   - Patrones armonicos (Gartley, Bat, Cypher)
   - Correlacion entre pares para confirmacion
   ============================================================
   
   FILTROS DE CALIDAD EXTREMA:
   - Minimo 3 de 3 estrategias alineadas para senal
   - Score combinado minimo 85% para DESTACADA
   - Score combinado minimo 90% para CONFIRMADA
   - Risk/Reward minimo 1:2.5 obligatorio
   - Volumen institucional confirmado (>1.5x promedio)
   - ATR en rango optimo (0.5% - 3%)
   - Sin senales en periodos de alta volatilidad
   
   SISTEMA DE CONFIANZA MEJORADO:
   - Red Neuronal con Attention Mechanism
   - Ensemble de 3 modelos especializados
   - Calibracion Bayesiana de probabilidades
   - Backtesting continuo para validacion
...
ULTIMA MODIFICACION: 05/02/2026 - TRIPLE ESTRATEGIA 85% WIN RATE
"""
...
print("Iniciando Crypto Bot Pro v36.0.0.0 - Triple Estrategia 85% Win Rate")
...
try:
    ...
    print("PyTorch disponible - Red Neuronal habilitada")
except ImportError as e:
    TORCH_AVAILABLE = False
    print(f"PyTorch no disponible: {e} - Funcionalidad de IA limitada")
# Importaciones para detección de patrones
try:
    from scipy.signal import argrelextrema
    SCIPY_AVAILABLE = True
    print("✅ SciPy disponible - Detección de patrones avanzada")
except ImportError as e:
    SCIPY_AVAILABLE = False
    print(f"⚠️ SciPy no disponible: {e} - Detección de patrones limitada")
...
class StrategyType(Enum):
    ...
    # === NUEVAS ESTRATEGIAS v36 - TRIPLE COMBINACION 85% ===
    MOMENTUM_INSTITUCIONAL = "MOMENTUM_INSTITUCIONAL"      # Estrategia 1
    ESTRUCTURA_MERCADO = "ESTRUCTURA_MERCADO"              # Estrategia 2
    CONFLUENCE_MTF = "CONFLUENCE_MTF"                       # Estrategia 3
    TRIPLE_ESTRATEGIA = "TRIPLE_ESTRATEGIA_85"             # Combinacion de las 3
...
class AdvancedTradingConfig:
    ...
    def __init__(self):
        ...
        # ============================================================
        # UMBRALES v36.0 - TRIPLE ESTRATEGIA 85% WIN RATE
        # ============================================================
        self.MIN_NEURAL_DESTACADA = 85.0      # IA minima para DESTACADA
        self.MIN_TECHNICAL_DESTACADA = 85.0   # Tecnico minimo para DESTACADA
        self.MIN_ALIGNMENT_DESTACADA = 85.0   # Alineacion minima para DESTACADA
        self.MIN_NEURAL_CONFIRMADA = 90.0     # IA minima para CONFIRMADA
        self.MIN_TECHNICAL_CONFIRMADA = 90.0  # Tecnico minimo para CONFIRMADA
        self.MIN_ALIGNMENT_CONFIRMADA = 90.0  # Alineacion minima para CONFIRMADA
        self.MIN_NEURAL_VALIDATION = 80.0
        
        # === TRIPLE ESTRATEGIA - CONFIGURACION v36 ===
        self.TRIPLE_STRATEGY_ENABLED = True   # Activar sistema triple
        self.MIN_STRATEGIES_ALIGNED = 3       # Minimo 3 de 3 estrategias alineadas
        self.MIN_COMBINED_SCORE = 85.0        # Score combinado minimo 85%
        
        # Pesos de cada estrategia
        self.STRATEGY_WEIGHT_MOMENTUM = 0.35      # 35% - Momentum Institucional
        self.STRATEGY_WEIGHT_ESTRUCTURA = 0.35   # 35% - Estructura de Mercado
        self.STRATEGY_WEIGHT_CONFLUENCE = 0.30   # 30% - Confluence MTF
        
        # Umbrales por estrategia
        self.MIN_MOMENTUM_SCORE = 80.0        # Score minimo Momentum
        self.MIN_ESTRUCTURA_SCORE = 80.0      # Score minimo Estructura
        self.MIN_CONFLUENCE_SCORE = 80.0      # Score minimo Confluence
        
        # Parametros de Momentum Institucional
        self.VOLUME_INSTITUTIONAL_MULTIPLIER = 1.5  # Volumen 1.5x para institucional
        self.VWAP_DEVIATION_THRESHOLD = 1.5         # Desviaciones estandar de VWAP
        self.ORDER_FLOW_DELTA_MIN = 0.6             # Delta minimo de flujo
        
        # Parametros de Estructura de Mercado
        self.BOS_CONFIRMATION_CANDLES = 3          # Velas para confirmar BOS
        self.CHOCH_CONFIRMATION_CANDLES = 2        # Velas para confirmar CHoCH
        self.FVG_MIN_SIZE_ATR = 0.5                # Tamano minimo FVG en ATR
        self.ORDER_BLOCK_LOOKBACK = 50             # Velas para buscar Order Blocks
        
        # Parametros de Confluence MTF
        self.MTF_TIMEFRAMES = ['5m', '15m', '30m', '1h']  # Timeframes para confluence
        self.MTF_ALIGNMENT_MIN = 0.75              # 75% de TFs alineados minimo
        self.FIBONACCI_RETRACEMENT_LEVELS = [0.236, 0.382, 0.5, 0.618, 0.786]
        self.FIBONACCI_EXTENSION_LEVELS = [1.0, 1.272, 1.618, 2.0, 2.618]
        
        # Configuracion original mejorada
        self.REQUIRE_CANDLE_PATTERN = True
        self.REQUIRE_ENTRY_SETUP = True
        self.ENTRY_EMA_PERIOD = 21
        self.MIN_ENTRY_PATTERN_CONFIDENCE = 80.0  # Aumentado a 80%
        self.MAX_ENTRY_DISTANCE_ATR = 0.8  # Reducido a 0.8 (mas preciso)
        self.MAX_ENTRY_CANDLE_RANGE_ATR = 1.2  # Reducido a 1.2 (rango estrecho)
        self.MAX_ENTRY_CANDLE_BODY_ATR = 0.7  # Reducido a 0.7 (cuerpo mas fuerte)
        self.ENTRY_PULLBACK_REQUIRED = True
        self.ENTRY_CONFLUENCE_BYPASS = 90  # Aumentado a 90 (mas selectivo)
        self.MIN_VOLATILITY_PERCENT = 0.5
        self.MAX_VOLATILITY_PERCENT = 3.0  # Nuevo: maximo volatilidad
        self.DYNAMIC_THRESHOLDS_ENABLED = True
        self.THRESHOLD_WINDOW_DAYS = 90
        self.THRESHOLD_MIN = 75.0
        ...
        # Validation parameters v36 - MAS ESTRICTOS
        self.MIN_TECH_VALIDATION = 85.0
        self.MIN_NEURAL_VALIDATION = 85.0
        self.MIN_VOLUME_RATIO = 1.5  # Aumentado a 1.5x (institucional)
        self.MIN_RISK_REWARD_RATIO = 2.5  # Aumentado a 1:2.5 para mejor profit
        self.MAX_RISK_PER_TRADE = 0.015  # Reducido a 1.5% por trade
        self.NEURAL_WEIGHT = 0.40  # Ajustado para triple estrategia
        self.TECHNICAL_WEIGHT = 0.30
        self.TRIPLE_STRATEGY_WEIGHT = 0.30  # Nuevo peso para triple estrategia
        ...
        # Trading parameters
        self.DEFAULT_STOP_LOSS_PERCENT = 0.005
        self.DEFAULT_TAKE_PROFIT_PERCENT = 0.01
        self.PROFIT_TARGET_PERCENT = 2.0  # Reducido de 3% a 2% (más realista)
        self.STOP_LOSS_PERCENT = 1.0
        self.MIN_STOP_LOSS_PERCENT = 1.0   # Piso mínimo SL (%) para baja volatilidad
        self.TAKE_PROFIT_PERCENT = 2.0     # Actualizado a 2% (realista)
        self.MIN_RISK_REWARD_RATIO = 1.5  # Aumentado a 1.5 (+50%)
        ...
        self.MIN_SIGNAL_SCORE = 0.75  # Score mínimo para aceptar (75%)
        self.MIN_CONFLUENCE = 0.60  # 3 de 5 indicadores alineados
...
class UnifiedMarketAnalyzer:
    ...
    def _fast_engine_lightweight(self, df: pd.DataFrame, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # RSI mínimo - RELAJADO para entradas tempranas
            rsi = float(latest.get('rsi', 50.0))
            rsi_buy_min = 42.0  # Was 52.0 or config
            rsi_sell_max = 58.0 # Was 48.0 or config
            if (direction == "BUY" and rsi < rsi_buy_min) or (direction == "SELL" and rsi > rsi_sell_max):
                return None

            # Timing - Permitir pullback leve a la EMA
            if (direction == "BUY" and price < ema_20 * 0.998) or (direction == "SELL" and price > ema_20 * 1.002):
                return None

            # Volatilidad (ATR ligero)
            df_atr = self._compute_atr_light(df, period=14)
            if df_atr is None:
                return None
            atr = float(df_atr.iloc[-1]['atr'])
    ...
    def _compute_atr_light(self, df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
        """
        Versión ligera de ATR (solo para FAST-FAIL).
        No requiere columnas adicionales ni validaciones complejas.
        """
        try:
            ...
            logger.error(f"Error en _compute_atr_light: {e}")
            return None
...
class OptimizedTechnicalAnalyzer:
    ...
    def calculate_dynamic_stop_loss(self, entry_price: float, atr: float, 
        ...
        """
        Calcula Stop Loss y Take Profit dinamicos basados en ATR.
        
        Mejora v35:
        - Trailing stop dinamico basado en ATR
        - Milestones de profit para mover SL a breakeven
        - Risk/Reward adaptativo segun volatilidad
        """
        try:
            # Pisos minimos basados en porcentaje (configurables)
            min_sl_pct = getattr(self.config, 'MIN_STOP_LOSS_PERCENT', 0.3) / 100
            min_tp_pct = getattr(self.config, 'TAKE_PROFIT_PERCENT', 1.0) / 100

            min_sl_distance = entry_price * min_sl_pct
            min_tp_distance = entry_price * min_tp_pct

            if atr <= 0 or entry_price <= 0:
                sl_distance = max(entry_price * 0.01, min_sl_distance)
                tp_distance = max(entry_price * 0.025, min_tp_distance)
                atr_multiplier_used = 0
            else:
                # ATR con pisos minimos (evita stops muy ajustados)
                sl_distance = max(atr * multiplier, min_sl_distance)
                tp_distance = max(atr * multiplier * 2.5, min_tp_distance)
            ...
            risk_reward = tp_percent / max(sl_percent, 0.01)
            
            # Milestones para trailing stop (mover SL a breakeven, luego profit)
            milestone_breakeven = entry_price * (1.005 if trend_direction == 'BULLISH' else 0.995)  # 0.5%
            milestone_profit_1 = entry_price * (1.01 if trend_direction == 'BULLISH' else 0.99)    # 1%
            milestone_profit_2 = entry_price * (1.015 if trend_direction == 'BULLISH' else 0.985)  # 1.5%
            ...
            return {
                ...
                'atr_based': atr > 0,
                'atr_multiplier': atr_multiplier_used,
                'trailing_stop_distance': sl_distance * 0.7,  # Trailing mas ajustado que SL inicial
                'milestone_breakeven': milestone_breakeven,
                'milestone_profit_1': milestone_profit_1,
                'milestone_profit_2': milestone_profit_2
            }
        except Exception as e:
            logger.debug(f"Error calculando SL/TP dinamico: {e}")
            return {
    ...
    def evaluate_entry_setup(self, df_entry: pd.DataFrame, market_direction: str, candle_pattern: dict,
        ...
        try:
            ...
            atr_val = float(atr) if atr is not None else float(self.calculate_atr(df_entry, 14))
            if atr_val <= 1e-12:
                return {'valid': True, 'grade': 'N/A', 'reason': 'ATR inválido', 'text': "🎯 Entrada: N/A (ATR inválido)"}

            ema_period = int(getattr(self.config, 'ENTRY_EMA_PERIOD', 21))
            ema_series = self.calculate_ema(df_entry['close'], ema_period, data_id + "_entry")
            ...
            min_pat = float(getattr(self.config, 'MIN_ENTRY_PATTERN_CONFIDENCE', 60.0))
            max_dist = float(getattr(self.config, 'MAX_ENTRY_DISTANCE_ATR', 1.2))
            max_range = float(getattr(self.config, 'MAX_ENTRY_CANDLE_RANGE_ATR', 1.6))
            max_body = float(getattr(self.config, 'MAX_ENTRY_CANDLE_BODY_ATR', 1.1))
            pullback_required = bool(getattr(self.config, 'ENTRY_PULLBACK_REQUIRED', True))
            confluence_bypass = float(getattr(self.config, 'ENTRY_CONFLUENCE_BYPASS', 75))
            ...
            logger.debug(f"Error en evaluate_entry_setup: {e}")
            return {'valid': True, 'grade': 'N/A', 'reason': f'Error: {e}', 'text': "🎯 Entrada: N/A (error interno)"}
        ...
        try:
            ...
            def _score_levels(indices, prices, levels_type='SUPPORT'):
                ...
                for i in indices:
                    ...
                    levels.append({
                        'price': price, 'index': int(i), 'retests': retests,
                        'age': age, 'volume': vol_at, 'strength': min(100.0, score),
                        'type': levels_type
                    })
                return levels
            ...
            for s in supports:
                ...
                if 0 > dist >= -0.03:  # soporte ≤3% por debajo
    ...
    def validate_ultra_selective_conditions(self, symbol: str,
        ...
        """
        ✅ VALIDACIÓN SIMPLIFICADA v36.0.0.0 – SOLO 4 CRITERIOS CLAVE
        ...
        try:
            ...
            atr = self.calculate_atr(df_entry, 14)
...
class OptimizedStrategyImplementation:
    ...
    def check_optimized_unified_strategy(self, df_primary: pd.DataFrame, df_entry: pd.DataFrame, neural_prediction: dict = None) -> Tuple[bool, float, TrendDirection, List[str], float]:
        ...
        sr_levels = self.technical_analyzer.find_support_resistance(df_entry)
        sr_score = 0

        if trend_direction == TrendDirection.BULLISH:
            if sr_levels['resistance'] is not None:
                distance_to_resistance = (sr_levels['resistance'] - current_price_entry) / current_price_entry
                if 0.01 < distance_to_resistance < 0.05:  # 1% a 5% por debajo de resistencia
                    sr_score = 30
                    conditions_met.append(f"Cerca de resistencia (Distancia: {distance_to_resistance:.2%})")
        elif trend_direction == TrendDirection.BEARISH:
            if sr_levels['support'] is not None:
                distance_to_support = (current_price_entry - sr_levels['support']) / current_price_entry
                if 0.01 < distance_to_support < 0.05:  # 1% a 5% por encima de soporte
                    sr_score = 30
                    conditions_met.append(f"Cerca de soporte (Distancia: {distance_to_support:.2%})")
...
# ============================================================================
# SISTEMA DE TRIPLE ESTRATEGIA v36.0.0.0 - 85% WIN RATE
# Combina 3 estrategias independientes para maxima precision
# ============================================================================
class TripleStrategyAnalyzer:
    """
    Sistema de Triple Estrategia para 85% de efectividad
    
    ESTRATEGIA 1: MOMENTUM INSTITUCIONAL (35%)
    - Detecta flujo de dinero institucional
    - Analiza volumen con VWAP
    - Divergencias RSI/MACD ocultas
    
    ESTRATEGIA 2: ESTRUCTURA DE MERCADO (35%)
    - Break of Structure (BOS)
    - Change of Character (CHoCH)
    - Order Blocks y Fair Value Gaps
    
    ESTRATEGIA 3: CONFLUENCE MULTI-TIMEFRAME (30%)
    - Alineacion de multiples timeframes
    - Niveles Fibonacci dinamicos
    - Patrones armonicos
    """
    ...
    def analyze_momentum_institucional(self, df: pd.DataFrame, symbol: str) -> dict:
        """
        ESTRATEGIA 1: Momentum Institucional
        Detecta flujo de dinero institucional mediante volumen y VWAP
        """
        result = {
            'strategy': 'MOMENTUM_INSTITUCIONAL',
            'score': 0.0,
            'direction': 'NEUTRAL',
            'signals': [],
            'confidence': 0.0,
        ...
        try:
            ...
            # 1. VWAP y desviaciones
            typical_price = (high + low + close) / 3
            cumulative_tpv = np.cumsum(typical_price * volume)
            cumulative_vol = np.cumsum(volume)
            vwap = cumulative_tpv / np.where(cumulative_vol > 0, cumulative_vol, 1)
            
            # Desviacion estandar de precio respecto a VWAP
            vwap_std = np.std(close[-20:] - vwap[-20:])
            current_deviation = (close[-1] - vwap[-1]) / vwap_std if vwap_std > 0 else 0
            ...
            # 3. Delta de volumen (aproximacion)
            price_change = close[-1] - close[-2]
            volume_delta = volume[-1] * np.sign(price_change)
            cumulative_delta = np.sum([volume[-i] * np.sign(close[-i] - close[-i-1]) for i in range(1, min(11, len(close)))])
            
            # 4. RSI para divergencias
            rsi = self._calculate_rsi(close, 14)
            
            # 5. MACD para divergencias
            ...
            # Divergencia alcista oculta: precio hace higher low, RSI hace lower low
            if len(close) > 5 and len(rsi) > 5:
                price_hl = close[-1] > close[-5] and low[-1] > low[-5]
                rsi_ll = rsi[-1] < rsi[-5]
                bullish_divergence = price_hl and rsi_ll
                
                # Divergencia bajista oculta: precio hace lower high, RSI hace higher high
                price_lh = close[-1] < close[-5] and high[-1] < high[-5]
                rsi_hh = rsi[-1] > rsi[-5]
                bearish_divergence = price_lh and rsi_hh
            
            # Calcular score
            score = 0.0
            signals = []
            
            # Score por volumen institucional
            if volume_ratio >= self.config.VOLUME_INSTITUTIONAL_MULTIPLIER:
                score += 25
                signals.append(f"Volumen institucional: {volume_ratio:.2f}x")
            elif volume_ratio >= 1.2:
                score += 15
                signals.append(f"Volumen elevado: {volume_ratio:.2f}x")
            
            # Score por posicion VWAP
            if abs(current_deviation) < 1.0:  # Cerca de VWAP
                score += 20
                signals.append(f"Precio en VWAP ({current_deviation:.2f} std)")
            elif current_deviation > 1.5:  # Por encima de VWAP
                score += 10
                signals.append(f"Precio sobre VWAP ({current_deviation:.2f} std)")
            elif current_deviation < -1.5:  # Por debajo de VWAP
                score += 10
                signals.append(f"Precio bajo VWAP ({current_deviation:.2f} std)")
            
            # Score por delta de volumen
            if cumulative_delta > 0:
                score += 15
                signals.append(f"Delta positivo: {cumulative_delta:.0f}")
                direction = 'BULLISH'
            elif cumulative_delta < 0:
                score += 15
                signals.append(f"Delta negativo: {cumulative_delta:.0f}")
                direction = 'BEARISH'
            else:
                direction = 'NEUTRAL'
            
            # Score por divergencias
            if bullish_divergence:
                score += 25
                signals.append("Divergencia alcista oculta RSI")
                direction = 'BULLISH'
            elif bearish_divergence:
                score += 25
                signals.append("Divergencia bajista oculta RSI")
                direction = 'BEARISH'
            ...
            if len(hist) > 1:
                if hist[-1] > hist[-2] > 0:
                    score += 15
                    signals.append("MACD momentum alcista")
                    if direction == 'NEUTRAL':
                        direction = 'BULLISH'
                elif hist[-1] < hist[-2] < 0:
                    score += 15
                    signals.append("MACD momentum bajista")
                    if direction == 'NEUTRAL':
                        direction = 'BEARISH'
            ...
            result['valid'] = score >= self.config.MIN_MOMENTUM_SCORE
            result['volume_ratio'] = volume_ratio
            result['vwap_deviation'] = current_deviation
            result['cumulative_delta'] = cumulative_delta
            
        except Exception as e:
            logger.error(f"Error en analyze_momentum_institucional: {e}")
            
        return result
    
    def analyze_estructura_mercado(self, df: pd.DataFrame, symbol: str) -> dict:
        """
        ESTRATEGIA 2: Estructura de Mercado
        Analiza BOS, CHoCH, Order Blocks y Fair Value Gaps
        """
        result = {
            'strategy': 'ESTRUCTURA_MERCADO',
            'score': 0.0,
            'direction': 'NEUTRAL',
            'signals': [],
            'confidence': 0.0,
        ...
        try:
            ...
            # 1. Encontrar swing highs y swing lows
            swing_highs, swing_lows = self._find_swing_points(high, low, 5)
            
            # 2. Detectar Break of Structure (BOS)
            bos_bullish, bos_bearish = self._detect_bos(close, swing_highs, swing_lows)
            
            # 3. Detectar Change of Character (CHoCH)
            choch_bullish, choch_bearish = self._detect_choch(close, swing_highs, swing_lows)
            
            # 4. Encontrar Order Blocks
            order_blocks = self._find_order_blocks(df)
            
            # 5. Encontrar Fair Value Gaps (FVG)
            fvg_bullish, fvg_bearish = self._find_fvg(df)
            ...
            if bos_bullish:
                score += 30
                signals.append("BOS Alcista confirmado")
                direction = 'BULLISH'
            elif bos_bearish:
                score += 30
                signals.append("BOS Bajista confirmado")
                direction = 'BEARISH'
            
            # Score por CHoCH (mas importante que BOS)
            if choch_bullish:
                score += 35
                signals.append("CHoCH Alcista - Cambio de caracter")
                direction = 'BULLISH'
            elif choch_bearish:
                score += 35
                signals.append("CHoCH Bajista - Cambio de caracter")
                direction = 'BEARISH'
            
            # Score por Order Blocks
            current_price = close[-1]
            for ob in order_blocks[-3:]:  # Ultimos 3 order blocks
                if ob['type'] == 'BULLISH' and ob['low'] <= current_price <= ob['high']:
                    score += 20
                    signals.append(f"En Order Block alcista ({ob['low']:.6f}-{ob['high']:.6f})")
                    if direction == 'NEUTRAL':
                        direction = 'BULLISH'
                elif ob['type'] == 'BEARISH' and ob['low'] <= current_price <= ob['high']:
                    score += 20
                    signals.append(f"En Order Block bajista ({ob['low']:.6f}-{ob['high']:.6f})")
                    if direction == 'NEUTRAL':
                        direction = 'BEARISH'
            
            # Score por FVG
            for fvg in fvg_bullish[-2:]:
                if fvg['low'] <= current_price <= fvg['high']:
                    score += 15
                    signals.append("Precio en FVG alcista")
                    if direction == 'NEUTRAL':
                        direction = 'BULLISH'
                        
            for fvg in fvg_bearish[-2:]:
                if fvg['low'] <= current_price <= fvg['high']:
                    score += 15
                    signals.append("Precio en FVG bajista")
                    if direction == 'NEUTRAL':
                        direction = 'BEARISH'
            ...
            result['signals'] = signals
            result['confidence'] = score
            result['valid'] = score >= self.config.MIN_ESTRUCTURA_SCORE
            result['bos'] = {'bullish': bos_bullish, 'bearish': bos_bearish}
            result['choch'] = {'bullish': choch_bullish, 'bearish': choch_bearish}
            result['order_blocks'] = order_blocks[-3:] if order_blocks else []
            
        except Exception as e:
            logger.error(f"Error en analyze_estructura_mercado: {e}")
            
        return result
    
    def analyze_confluence_mtf(self, df_dict: dict, symbol: str) -> dict:
        """
        ESTRATEGIA 3: Confluence Multi-Timeframe
        Analiza alineacion de multiples timeframes y niveles Fibonacci
        """
        result = {
            'strategy': 'CONFLUENCE_MTF',
            'score': 0.0,
            'direction': 'NEUTRAL',
            'signals': [],
            'confidence': 0.0,
        ...
        try:
            # Necesitamos al menos 2 timeframes
            if not df_dict or len(df_dict) < 2:
                return result
            
            # Analizar tendencia en cada timeframe
            tf_trends = {}
            for tf, df in df_dict.items():
                if df is not None and len(df) >= 50:
                    trend = self._analyze_tf_trend(df)
                    tf_trends[tf] = trend
            ...
            # Calcular alineacion
            bullish_count = sum(1 for t in tf_trends.values() if t == 'BULLISH')
            bearish_count = sum(1 for t in tf_trends.values() if t == 'BEARISH')
            total = len(tf_trends)
            
            alignment_ratio = max(bullish_count, bearish_count) / total
            ...
            # Score por alineacion MTF
            if alignment_ratio >= 1.0:  # 100% alineado
                score += 40
                signals.append(f"MTF 100% alineado ({direction})")
            elif alignment_ratio >= 0.75:  # 75% alineado
                score += 30
                signals.append(f"MTF 75% alineado ({direction})")
            elif alignment_ratio >= 0.5:  # 50% alineado
                score += 15
                signals.append(f"MTF 50% alineado (mixto)")
            
            # Analizar Fibonacci en timeframe principal
            main_tf = '15m' if '15m' in df_dict else list(df_dict.keys())[0]
            if main_tf in df_dict and df_dict[main_tf] is not None:
                fib_analysis = self._analyze_fibonacci(df_dict[main_tf])
                if fib_analysis['near_level']:
                    score += 25
                    signals.append(f"Precio en Fib {fib_analysis['level']:.3f}")
                    
                if fib_analysis['extension_target']:
                    score += 15
                    signals.append(f"Extension Fib: {fib_analysis['extension']:.3f}")
            
            # Analizar patrones armonicos (simplificado)
            if main_tf in df_dict and df_dict[main_tf] is not None:
                harmonic = self._detect_harmonic_pattern(df_dict[main_tf])
                if harmonic['detected']:
                    score += 20
                    signals.append(f"Patron armonico: {harmonic['pattern']}")
                    if harmonic['direction'] != 'NEUTRAL':
            ...
            result['signals'] = signals
            result['confidence'] = score
            result['valid'] = score >= self.config.MIN_CONFLUENCE_SCORE
            result['alignment_ratio'] = alignment_ratio
            result['tf_trends'] = tf_trends
            
        except Exception as e:
            logger.error(f"Error en analyze_confluence_mtf: {e}")
            
        return result
    
    def calculate_combined_score(self, momentum: dict, estructura: dict, confluence: dict) -> dict:
        """
        Combina los scores de las 3 estrategias con ponderacion
        """
        result = {
            'combined_score': 0.0,
            'direction': 'NEUTRAL',
        ...
        try:
            # Ponderacion de cada estrategia
            w1 = self.config.STRATEGY_WEIGHT_MOMENTUM    # 0.35
            w2 = self.config.STRATEGY_WEIGHT_ESTRUCTURA  # 0.35
            w3 = self.config.STRATEGY_WEIGHT_CONFLUENCE  # 0.30
            
            # Calcular score combinado ponderado
            score1 = momentum.get('score', 0) * w1
            score2 = estructura.get('score', 0) * w2
            score3 = confluence.get('score', 0) * w3
            
            combined_score = score1 + score2 + score3
            
            # Contar estrategias alineadas en la misma direccion
            directions = []
            if momentum.get('valid', False):
                directions.append(momentum.get('direction', 'NEUTRAL'))
            if estructura.get('valid', False):
                directions.append(estructura.get('direction', 'NEUTRAL'))
            if confluence.get('valid', False):
                directions.append(confluence.get('direction', 'NEUTRAL'))
            
            # Remover NEUTRAL para contar alineacion
            valid_directions = [d for d in directions if d != 'NEUTRAL']
            
            if valid_directions:
                bullish = valid_directions.count('BULLISH')
                bearish = valid_directions.count('BEARISH')
                
                if bullish > bearish:
                    direction = 'BULLISH'
                    strategies_aligned = bullish
                elif bearish > bullish:
                    direction = 'BEARISH'
                    strategies_aligned = bearish
                else:
                    direction = 'NEUTRAL'
                    strategies_aligned = 0
            else:
            ...
            # Validar si cumple requisitos minimos
            is_valid = (
                combined_score >= self.config.MIN_COMBINED_SCORE and
                strategies_aligned >= self.config.MIN_STRATEGIES_ALIGNED
            )
            
            # Compilar senales
            all_signals = []
            all_signals.extend(momentum.get('signals', []))
            all_signals.extend(estructura.get('signals', []))
            all_signals.extend(confluence.get('signals', []))
            
            result['combined_score'] = combined_score
            result['direction'] = direction
            result['strategies_aligned'] = strategies_aligned
            result['is_valid'] = is_valid
            result['signals'] = all_signals[:10]  # Limitar a 10 senales
            result['breakdown'] = {
                'momentum': {'score': momentum.get('score', 0), 'weighted': score1, 'valid': momentum.get('valid', False)},
                'estructura': {'score': estructura.get('score', 0), 'weighted': score2, 'valid': estructura.get('valid', False)},
                'confluence': {'score': confluence.get('score', 0), 'weighted': score3, 'valid': confluence.get('valid', False)}
            }
            
        except Exception as e:
            logger.error(f"Error en calculate_combined_score: {e}")
            
        return result
    
    def full_analysis(self, df_primary: pd.DataFrame, df_entry: pd.DataFrame, 
                      df_dict: dict, symbol: str) -> dict:
        """
        Ejecuta analisis completo de las 3 estrategias
        """
        # Estrategia 1: Momentum Institucional
        momentum = self.analyze_momentum_institucional(df_entry, symbol)
        
        # Estrategia 2: Estructura de Mercado
        estructura = self.analyze_estructura_mercado(df_entry, symbol)
        
        # Estrategia 3: Confluence MTF
        confluence = self.analyze_confluence_mtf(df_dict, symbol)
        
        # Combinar scores
        combined = self.calculate_combined_score(momentum, estructura, confluence)
        ...
        return {
            ...
            'combined': combined,
            'is_triple_valid': combined['is_valid'],
            'triple_score': combined['combined_score'],
            'triple_direction': combined['direction']
        }
    ...
    def _find_swing_points(self, high: np.ndarray, low: np.ndarray, order: int = 5) -> Tuple[list, list]:
    ...
    def _detect_bos(self, close: np.ndarray, swing_highs: list, swing_lows: list) -> Tuple[bool, bool]:
        ...
        if len(swing_highs) >= 2 and len(swing_lows) >= 2:
            ...
            # BOS alcista: precio rompe ultimo swing high
            if current_price > last_high:
                bos_bullish = True
            # BOS bajista: precio rompe ultimo swing low
            if current_price < last_low:
                bos_bearish = True
        
        return bos_bullish, bos_bearish
    
    def _detect_choch(self, close: np.ndarray, swing_highs: list, swing_lows: list) -> Tuple[bool, bool]:
        """Detecta Change of Character"""
        choch_bullish = False
        choch_bearish = False
        
        if len(swing_highs) >= 3 and len(swing_lows) >= 3:
            # CHoCH alcista: Higher Low despues de Lower Lows
            if (swing_lows[-1]['price'] > swing_lows[-2]['price'] and 
                swing_lows[-2]['price'] < swing_lows[-3]['price']):
                choch_bullish = True
            
            # CHoCH bajista: Lower High despues de Higher Highs
    ...
    def _find_order_blocks(self, df: pd.DataFrame) -> list:
        ...
        for i in range(3, len(df) - 1):
            # Order Block alcista: vela bajista fuerte seguida de movimiento alcista
            if close[i] < open_[i]:  # Vela bajista
                future_high = max(high[i+1:min(i+5, len(high))])
                if future_high > high[i]:  # Movimiento alcista posterior
                    order_blocks.append({
                        'type': 'BULLISH',
                        'index': i,
                        'high': high[i],
            ...
            # Order Block bajista: vela alcista fuerte seguida de movimiento bajista
            if close[i] > open_[i]:  # Vela alcista
                future_low = min(low[i+1:min(i+5, len(low))])
                if future_low < low[i]:  # Movimiento bajista posterior
                    order_blocks.append({
                        'type': 'BEARISH',
                        'index': i,
                        'high': high[i],
    ...
    def _find_fvg(self, df: pd.DataFrame) -> Tuple[list, list]:
        """Encuentra Fair Value Gaps"""
        fvg_bullish = []
        fvg_bearish = []
        ...
        for i in range(2, len(df)):
            # FVG alcista: gap entre high de vela i-2 y low de vela i
            if low[i] > high[i-2]:
                fvg_bullish.append({
                    'index': i,
                    'low': high[i-2],
            ...
            # FVG bajista: gap entre low de vela i-2 y high de vela i
            if high[i] < low[i-2]:
                fvg_bearish.append({
                    'index': i,
                    'low': high[i],
    ...
    def _analyze_tf_trend(self, df: pd.DataFrame) -> str:
        """Analiza tendencia en un timeframe"""
        if df is None or len(df) < 50:
            return 'NEUTRAL'
    ...
    def _analyze_fibonacci(self, df: pd.DataFrame) -> dict:
        """Analiza niveles Fibonacci"""
        result = {
            'near_level': False,
            'level': 0,
        ...
        try:
            ...
            # Verificar si esta cerca de un nivel Fibonacci
            for level in self.config.FIBONACCI_RETRACEMENT_LEVELS:
                fib_price = recent_high - (diff * level)
                if abs(current_price - fib_price) / current_price < 0.005:  # 0.5% de tolerancia
                    result['near_level'] = True
                    result['level'] = level
                    break
            
            # Calcular extension
            for ext in self.config.FIBONACCI_EXTENSION_LEVELS:
                ext_price = recent_low + (diff * ext)
                if current_price < ext_price:
                    result['extension_target'] = True
                    result['extension'] = ext
                    break
                    
        except Exception as e:
            logger.debug(f"Error en Fibonacci: {e}")
        
        return result
    
    def _detect_harmonic_pattern(self, df: pd.DataFrame) -> dict:
        """Detecta patrones armonicos simplificados"""
        result = {
            'detected': False,
            'pattern': '',
            'direction': 'NEUTRAL'
        ...
        try:
            ...
            # Simplificacion: buscar patron AB=CD
            if len(close) < 20:
                return result
            
            # Encontrar pivotes recientes
            swing_highs, swing_lows = self._find_swing_points(high, low, 3)
            
            if len(swing_highs) >= 2 and len(swing_lows) >= 2:
                # Verificar si hay simetria (AB=CD simplificado)
                last_moves = []
                all_swings = sorted(
                    [{'type': 'H', **h} for h in swing_highs[-3:]] + 
                    [{'type': 'L', **l} for l in swing_lows[-3:]],
                    key=lambda x: x['index']
                )
                
                if len(all_swings) >= 4:
                    ab = abs(all_swings[1]['price'] - all_swings[0]['price'])
                    cd = abs(all_swings[3]['price'] - all_swings[2]['price'])
                    
                    # Si AB es similar a CD (dentro del 20%)
                    if 0.8 <= cd / ab <= 1.2 if ab > 0 else False:
                        result['detected'] = True
                        result['pattern'] = 'AB=CD'
                        
                        # Determinar direccion
                        if all_swings[3]['type'] == 'L':
                            result['direction'] = 'BULLISH'
                        else:
                            result['direction'] = 'BEARISH'
                            
        except Exception as e:
            logger.debug(f"Error detectando patron armonico: {e}")
        
        return result

# ============================================================================
# SISTEMA DE PROCESAMIENTO DE SENALES OPTIMIZADO v36.0.0.0
# Con Triple Estrategia, filtros de calidad y validacion mejorada
# ============================================================================
class OptimizedSignalProcessor:
    def __init__(self, config: "AdvancedTradingConfig"):
        ...
        # === NUEVO v36: Triple Strategy Analyzer ===
        self.triple_strategy = TripleStrategyAnalyzer(config)
        
        # === MEJORAS v36: Sistema de cooldown y filtros mejorados ===
        self.signal_cooldown = {}  # {symbol: timestamp} - Cooldown entre senales del mismo par
        self.COOLDOWN_MINUTES = 45  # Aumentado a 45 min para mayor selectividad
        
        # Cache de senales recientes para evitar duplicados
        self.recent_signals = {}  # {symbol: {'direction': str, 'timestamp': datetime}}
        
        # Estadisticas de calidad de senales v36
        self.signal_quality_stats = {
            ...
            'filtered_cooldown': 0,
            'filtered_quality': 0,
            'filtered_triple_strategy': 0,  # Nuevo: filtrado por triple estrategia
            'approved': 0,
            'triple_strategy_approved': 0   # Nuevo: aprobadas con triple estrategia
        }
        
        # Umbrales de calidad minima v36 - MAS ESTRICTOS
        self.MIN_VOLUME_RATIO = 1.5  # Aumentado a 1.5x (institucional)
        self.MIN_ATR_PERCENT = 0.5   # Aumentado a 0.5% minimo
        self.MAX_ATR_PERCENT = 3.0   # Reducido a 3% maximo
        self.MIN_RR_RATIO = 2.5      # Aumentado a 1:2.5 Risk/Reward
    ...
    def validate_signal_quality(self, symbol: str, validation_result: dict, 
        ...
        # 6. NUEVO v36: Check Triple Estrategia
        triple_result = validation_result.get('triple_strategy_result')
        if triple_result and self.config.TRIPLE_STRATEGY_ENABLED:
            if not triple_result.get('is_triple_valid', False):
                self.signal_quality_stats['filtered_triple_strategy'] += 1
                reasons.append(f"Triple Estrategia: {triple_result.get('triple_score', 0):.1f}% (min: {self.config.MIN_COMBINED_SCORE}%)")
                is_valid = False
            else:
                self.signal_quality_stats['triple_strategy_approved'] += 1
                reasons.append(f"Triple Estrategia OK: {triple_result.get('triple_score', 0):.1f}%")
                reasons.append(f"  - Momentum: {triple_result.get('momentum', {}).get('score', 0):.0f}%")
                reasons.append(f"  - Estructura: {triple_result.get('estructura', {}).get('score', 0):.0f}%")
                reasons.append(f"  - Confluence: {triple_result.get('confluence', {}).get('score', 0):.0f}%")
    ...
    def get_quality_stats(self) -> dict:
        """Retorna estadisticas de filtrado de senales v36"""
        total = self.signal_quality_stats['total_generated']
        if total > 0:
            approval_rate = (self.signal_quality_stats['approved'] / total) * 100
            triple_rate = (self.signal_quality_stats.get('triple_strategy_approved', 0) / total) * 100
        else:
            approval_rate = 0
            triple_rate = 0
        ...
        return {
            **self.signal_quality_stats,
            ...
            'triple_strategy_rate': f"{triple_rate:.1f}%",
            'version': 'v36.0.0.0 - Triple Estrategia 85%'
        }
    
    def analyze_with_triple_strategy(self, symbol: str, df_primary: pd.DataFrame, 
                                      df_entry: pd.DataFrame, df_dict: dict = None) -> dict:
        """
        Analiza un par usando el sistema de Triple Estrategia v36
        Returns: dict con resultado del analisis completo
        """
        try:
            if df_dict is None:
                df_dict = {
            ...
            # Ejecutar analisis triple
            triple_result = self.triple_strategy.full_analysis(
                df_primary, df_entry, df_dict, symbol
            )
            
            return triple_result
            
        except Exception as e:
            logger.error(f"Error en analyze_with_triple_strategy: {e}")
            return {
    ...
    def process_optimized_signal(self,
        ...
        if market_data and self.trend_alignment_validator:
            ...
            if df_15m is not None and df_30m is not None:
                alignment = self.validate_trend_alignment.validate_trend_alignment(df_15m, df_30m)
                alignment_percentage = alignment.get('alignment_score', 0.0)
...
# ========== CLIENTE TELEGRAM OPTIMIZADO v36.0.0.0 ==========
class OptimizedTelegramClient:
    ...
    def send_milestone_update(self, symbol: str, milestone: float, profit: float, chart_path: str = None):
        ...
        try:
            ...
            logger.error(f"Error enviando hito {milestone}%: {e}")
            return False
...
class RobustWebSocketManager:
    """
    Gestor WebSocket Mejorado v36.0.0.0 con:
...
class SignalTracker:
    ...
    def __init__(self, config=None):
        ...
        self.validation_config = {
            ...
            'highlight_timeout_min': 20,
            'confirmed_timeout_min': 180,
    ...
    def promote_to_confirmed(self, signal_hash: str, current_neural_conf: float = None,
        ...
        with self.lock:
            ...
            if current_price and current_price > 0:
                ...
                tracking['entry_price'] = current_price  # Actualizar también entry_price principal
                # Recalcular SL/TP basado en nuevo precio de entrada
                ...
                # Si no hay precio actual, usar el precio de DESTACADA
    ...
    def close_signal(self, signal_hash: str, current_price: float, reason: str = "MANUAL") -> Optional[Dict]:
        with self.lock:
            ...
            result = reason
            if result == "PROFIT_TARGET" or result == "PARTIAL_TARGET":
                self.performance_metrics['successful_signals'] += 1

            self.performance_metrics['closure_reasons'][result] += 1
    ...
    def get_signal_cycle_summary(self, signal_hash: str) -> dict:
        """
        Obtiene un resumen completo del ciclo de vida de una senal (NUEVO v35)
        
        Ciclo completo: DESTACADA -> CONFIRMADA -> Milestones -> Cierre
        
        Returns:
            dict con estado actual, tiempos, profit, milestones alcanzados
        """
        with self.lock:
            if signal_hash not in self.tracked_signals:
                return {'exists': False, 'error': 'Senal no encontrada'}
            ...
            highlight_time = tracking.get('highlight_start_time')
            confirmed_time = tracking.get('confirmed_start_time')
            
            elapsed_total = 0
            elapsed_destacada = 0
            elapsed_confirmada = 0
            
            now = datetime.now()
            
            if start_time:
                elapsed_total = (now - start_time).total_seconds() / 60
            
            if highlight_time:
                if confirmed_time:
                    elapsed_destacada = (confirmed_time - highlight_time).total_seconds() / 60
                else:
                    elapsed_destacada = (now - highlight_time).total_seconds() / 60
            
            if confirmed_time:
                elapsed_confirmada = (now - confirmed_time).total_seconds() / 60
            
            # Obtener milestones enviados
            milestones_sent = tracking.get('telegram_updates_sent', 0)
            milestones_config = getattr(self.config, 'PROFIT_MILESTONES', [0.5, 1.0, 1.5])
            milestones_reached = milestones_config[:milestones_sent] if milestones_sent > 0 else []
            
            # Obtener max profit alcanzado
            max_profit = tracking.get('max_profit', 0)
            current_profit = tracking.get('current_profit', 0)
            
            return {
                'exists': True,
                'signal_hash': signal_hash[:8],
                'symbol': signal_data.get('symbol', 'UNKNOWN'),
                'status': tracking.get('status', 'DESCONOCIDO'),
                'is_buy': tracking.get('is_buy', True),
                'direction': 'LONG' if tracking.get('is_buy', True) else 'SHORT',
                
                # Precios
                'entry_price': tracking.get('entry_price', 0),
                'entry_price_confirmada': tracking.get('entry_price_confirmada', 0),
                'current_price': tracking.get('current_price', 0),
                'stop_loss': signal_data.get('stop_loss', 0),
                'take_profit': signal_data.get('take_profit', 0),
                ...
                # Tiempos
                'elapsed_total_min': round(elapsed_total, 1),
                'elapsed_destacada_min': round(elapsed_destacada, 1),
                'elapsed_confirmada_min': round(elapsed_confirmada, 1),
                
                # Ciclo
                'fase_actual': self._get_fase_description(tracking.get('status')),
                'milestones_enviados': milestones_sent,
                'milestones_alcanzados': milestones_reached,
                'milestones_pendientes': milestones_config[milestones_sent:] if milestones_sent < len(milestones_config) else [],
                
                # Scores
                'neural_score': signal_data.get('neural_score', 0),
                'technical_score': signal_data.get('technical_percentage', 0),
                'alignment_score': tracking.get('alignment_score', signal_data.get('alignment_percentage', 0)),
                
                # Telegram
                'telegram_destacada_sent': tracking.get('promotion_telegram_sent', False) or tracking.get('initial_telegram_sent', False),
                'telegram_confirmada_sent': tracking.get('promotion_telegram_sent', False),
                'telegram_milestones_sent': milestones_sent
            }
    
    def _get_fase_description(self, status: str) -> str:
        """Descripcion de la fase actual del ciclo"""
        fases = {
            'DESTACADA': 'Fase 1/4: Senal detectada, esperando confirmacion',
            'CONFIRMADA': 'Fase 2/4: Confirmada, seguimiento activo de profit',
            'MILESTONE': 'Fase 3/4: Milestone alcanzado, acercandose a objetivo',
            'CERRADA': 'Fase 4/4: Ciclo completado'
        }
        return fases.get(status, f'Estado: {status}')
    ...
    # 🔥 MÉTODO CRÍTICO FALTANTE: PROMOCIÓN AUTOMÁTICA
    def check_promotion_timer(self) -> List[str]:
        """
        Verifica señales DESTACADA para promoción o cancelación.
        Returns:
            List[str]: Lista de signal_hashes procesados
        """
        processed = []
        promo_timeout_seconds = self.validation_config['highlight_timeout_min'] * 60
        ...
        with self.lock:
            ...
            for signal_hash, tracking in signals_copy:
                ...
                if elapsed >= promo_timeout_seconds:
                    ...
                    self.cancel_signal(signal_hash, "Timeout de promoción")
                    processed.append(signal_hash)
                    continue
                ...
                if elapsed >= min_promo_time_seconds:
                    ...
                    if neural >= min_neural and technical >= min_tech and alignment >= min_align:
                        ...
                        if self.promote_to_confirmed(signal_hash, current_price=current_price):
                            processed.append(signal_hash)
                            logger.info(f"🚀 Señal {signal_hash[:8]} promovida a CONFIRMADA @ ${current_price:.6f}")

        return processed

# ============================================================================
# BOT PRINCIPAL OPTIMIZADO (CORREGIDO v36.0.0.0)
# ============================================================================
class OptimizedTradingBot:
    ...
    def _release_exclusive_mode(self):
        """Liberar modo exclusivo y reanudar escaneo global."""
        if self.exclusive_tracking_mode:
            self.exclusive_tracking_mode = False
            self.tracked_symbol = None
            self.tracked_signal_hash = None
            self.single_active_signal_hash = None  # ← Asegurar limpieza total
            logger.debug("🔓 Modo exclusivo liberado")

            # ✅ CRÍTICO: Notificar al SymbolScanner para reanudar escaneo de TODOS los símbolos
            if hasattr(self, 'symbol_scanner') and self.symbol_scanner:
                try:
                    self.symbol_scanner.resume_all_scanning()
                    logger.info("▶️ Escaneo global reanudado tras liberar modo exclusivo")
                except Exception as e:
       ...
       ✅ CORRECCIÓN #1: Timer de promoción integrado
        ...
        while self.running:
            try:
                ...
                # ✅ CORRECCIÓN #1: VERIFICAR TIMER DE PROMOCIÓN CADA 15 SEGUNDOS
                current_time = time.time()
                if current_time - last_promotion_check >= PROMOTION_CHECK_INTERVAL:
                    processed = self.signal_tracker.check_promotion_timer()
                    if processed:
                        logger.info(f"⏰ Timer de promoción: {len(processed)} señales procesadas")
                ...
                if status == 'DESTACADA':
                    ...
                    if not start_time:
                        ...
                        if time_condition and thresholds_condition:
                            ...
                            if promoted:
                                status = 'CONFIRMADA'
                                promo_reason = f"Umbrales OK (IA:{neural:.0f}% Téc:{technical:.0f}% Alin:{alignment_score:.0f}%)"
                ...
                # ---------- 4B. Progreso de profit para CONFIRMADA ----------
                    ...
                    # ✅ CORRECCIÓN: Verificar que la señal esté CONFIRMADA antes de enviar milestones
                    if status == 'CONFIRMADA':
                        milestones = self.config.PROFIT_MILESTONES  # [0.5, 1.0, 1.5]
                        with self.signal_tracker.lock:
                            live_tracking = self.signal_tracker.tracked_signals.get(signal_hash, {})
                            updates_sent = live_tracking.get('telegram_updates_sent', 0)

                        logger.debug(f"📊 [MILESTONE CHECK] {symbol}: profit={profit_percent:.2f}%, milestones={milestones}, updates_sent={updates_sent}")
                        ...
                        for i, milestone in enumerate(milestones):
                            ...
                            logger.debug(f"  [M{i+1}] Comparando: profit={profit_percent:.2f}% vs milestone={milestone}% | updates_sent={updates_sent} vs i={i}")
                            if profit_percent >= milestone and updates_sent <= i:
                                with self.signal_tracker.lock:
                        ...
                        logger.debug(f"⏭️ {symbol} - Milestones solo para CONFIRMADA (estado actual: {status})")
            ...
            if self.config.telegram_enabled:
                try:
                    ...
                    self.telegram_client.send_closure_update(
                        ...
                        duration_minutes=report.get('duration_minutes', 0),
                        max_profit=report.get('max_profit_reached', 0.0),
                        chart_path=chart_path
                    )
    ...
    def _on_signal_closed_callback(self, symbol: str, report: dict):
        ...
        self._safe_gui_queue_put(('clear_signal_display', None))    # Limpiar panel principal
    ...
    def _release_exclusive_mode(self):
        """Libera el modo exclusivo y notifica a la GUI"""
        if self.exclusive_tracking_mode:
            self.exclusive_tracking_mode = False
            self.tracked_symbol = None
            self.tracked_signal_hash = None
            self._safe_gui_queue_put(('log_message', "▶️ MODO EXCLUSIVO LIBERADO: Reanudando escaneo"))
    ...
    def start_optimized(self):
        ...
        try:
            self.running = True
            logger.info("Bot Optimizado v36.0.0.0 iniciando...")
            ...
            self._safe_gui_queue_put(('log_message', "Bot Avanzado Optimizado v36.0.0.0 iniciando sistema..."))
            ...
            self._safe_gui_queue_put(('log_message', "✅ Bot Optimizado v36.0.0.0 iniciado correctamente"))
...
class SignalTracker:
    ...
    # METODO CRITICO: PROMOCION AUTOMATICA
    def check_promotion_timer(self) -> List[str]:
        """
        Verifica señales DESTACADA para promoción o cancelación.
        Returns:
...
class OptimizedCryptoBotGUI(QtWidgets.QMainWindow):
    def __init__(self, config_instance):
        super().__init__()
        try:
            self.setWindowTitle("Crypto Bot Pro v36.0.0.0 - Triple Estrategia 85% Win Rate - Propiedad Intelectual de Marketeradolfo")
            ...
            self._init_tabs()
            # Load initial data
            self.log_message("🤖 Bienvenido a Crypto Bot Pro v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE")
            self.log_message("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            self.log_message("📊 ESTRATEGIA 1: Momentum Institucional (35%) - Flujo de dinero + VWAP")
            self.log_message("📈 ESTRATEGIA 2: Estructura de Mercado (35%) - BOS/CHoCH + Order Blocks")
            self.log_message("🎯 ESTRATEGIA 3: Confluence MTF (30%) - Alineacion Multi-Timeframe")
            self.log_message("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            self.log_message("✅ Requisitos: 3/3 Estrategias Alineadas + Score >= 85% + R/R >= 1:2.5")
            self.log_message("👨‍💻 Desarrollado por Lic: Adolfo Daniel Aguirre")
            self.log_message("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    def _show_about_dialog(self):
        ...
        about_text = """
<h2 style='color: #00d4aa;'>Crypto Bot Pro v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE</h2>
...
<h3 style='color: #52b788;'>Sistema Triple Estrategia</h3>
<p>La version v36.0.0.0 implementa un sistema de <b>Triple Estrategia</b> 
diseñado para alcanzar una rentabilidad consistente del <b>85%</b> combinando 
tres metodologias de analisis independientes.</p>

<h3 style='color: #FFD700;'>ESTRATEGIA 1: Momentum Institucional (35%)</h3>
<ul>
<li><b>VWAP:</b> Precio Promedio Ponderado por Volumen</li>
<li><b>Order Flow:</b> Analisis de flujo de ordenes</li>
<li><b>Divergencias:</b> RSI/MACD ocultas</li>
<li><b>Volumen:</b> Deteccion institucional (>1.5x promedio)</li>
</ul>

<h3 style='color: #FFD700;'>ESTRATEGIA 2: Estructura de Mercado (35%)</h3>
<ul>
<li><b>BOS:</b> Break of Structure - Quiebre de estructura</li>
<li><b>CHoCH:</b> Change of Character - Cambio de caracter</li>
<li><b>Order Blocks:</b> Zonas institucionales</li>
<li><b>FVG:</b> Fair Value Gaps - Gaps de valor justo</li>
</ul>

<h3 style='color: #FFD700;'>ESTRATEGIA 3: Confluence MTF (30%)</h3>
<ul>
<li><b>Multi-Timeframe:</b> Alineacion 5m/15m/30m/1h</li>
<li><b>Fibonacci:</b> Retrocesos y extensiones dinamicas</li>
<li><b>Patrones Armonicos:</b> AB=CD, Gartley, Bat</li>
<li><b>Correlacion:</b> Entre pares para confirmacion</li>
</ul>

<h3 style='color: #52b788;'>Requisitos para Senal Valida</h3>
<ul>
<li><b>Score Combinado:</b> >= 85% (ponderado)</li>
<li><b>Estrategias Alineadas:</b> 3 de 3 en misma direccion</li>
<li><b>Risk/Reward:</b> >= 1:2.5</li>
<li><b>Volumen:</b> >= 1.5x promedio (institucional)</li>
<li><b>Volatilidad:</b> ATR 0.5% - 3% (rango optimo)</li>
</ul>
...
"""
    ...
    def _init_header(self):
        ...
        title_label = QtWidgets.QLabel("🚀 Crypto Bot Pro v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE - Lic: Adolfo Daniel Aguirre")
    ...
    def _init_tabs(self):
        ...
        self.triple_strategy_tab = self._create_triple_strategy_tab()
        self.notebook.addTab(self.triple_strategy_tab, "🏆 Triple Estrategia")
    ...
    def _create_triple_strategy_tab(self):
        """Crear pestaña de Triple Estrategia v36"""
        ...
        title_frame.setStyleSheet("background-color: #1a2a4c; border-radius: 10px; padding: 10px;")
        title_layout = QtWidgets.QVBoxLayout(title_frame)
        
        title_label = QtWidgets.QLabel("🏆 TRIPLE ESTRATEGIA v36.0.0.0 - 85% WIN RATE")
        title_label.setFont(QtGui.QFont("Arial", 16, QtGui.QFont.Bold))
        title_label.setAlignment(QtCore.Qt.AlignCenter)
        title_label.setStyleSheet("color: #FFD700;")
        title_layout.addWidget(title_label)
        
        subtitle = QtWidgets.QLabel("Combinacion de 3 estrategias independientes para maxima precision")
        subtitle.setAlignment(QtCore.Qt.AlignCenter)
        subtitle.setStyleSheet("color: #b0b0b0;")
        title_layout.addWidget(subtitle)
        
        tab_layout.addWidget(title_frame)
        
        # Grid de las 3 estrategias
        strategies_layout = QtWidgets.QHBoxLayout()
        
        # === ESTRATEGIA 1: MOMENTUM INSTITUCIONAL ===
        momentum_frame, momentum_layout = self._create_titled_frame("📊 ESTRATEGIA 1: Momentum Institucional (35%)", tab_widget)
        momentum_frame.setMinimumWidth(300)
        
        self.momentum_score_label = QtWidgets.QLabel("Score: 0%")
        self.momentum_score_label.setFont(QtGui.QFont("Arial", 20, QtGui.QFont.Bold))
        self.momentum_score_label.setStyleSheet("color: #00d4aa;")
        self.momentum_score_label.setAlignment(QtCore.Qt.AlignCenter)
        momentum_layout.addWidget(self.momentum_score_label)
        
        self.momentum_direction_label = QtWidgets.QLabel("Direccion: NEUTRAL")
        self.momentum_direction_label.setStyleSheet("color: #b0b0b0; font-size: 12px;")
        momentum_layout.addWidget(self.momentum_direction_label)
        
        momentum_details = QtWidgets.QLabel(
            "• VWAP: Precio vs promedio ponderado\n"
            "• Order Flow: Delta de volumen\n"
            "• Divergencias RSI/MACD ocultas\n"
            "• Volumen institucional (>1.5x)"
        )
        momentum_details.setStyleSheet("color: #808080; font-size: 10px;")
        momentum_layout.addWidget(momentum_details)
        
        self.momentum_signals_text = QtWidgets.QTextEdit()
        self.momentum_signals_text.setMaximumHeight(100)
        self.momentum_signals_text.setReadOnly(True)
        self.momentum_signals_text.setStyleSheet("background-color: #0f0f23; color: #00d4aa; border: 1px solid #1a2a4c;")
        momentum_layout.addWidget(self.momentum_signals_text)
        
        strategies_layout.addWidget(momentum_frame)
        
        # === ESTRATEGIA 2: ESTRUCTURA DE MERCADO ===
        estructura_frame, estructura_layout = self._create_titled_frame("📈 ESTRATEGIA 2: Estructura de Mercado (35%)", tab_widget)
        estructura_frame.setMinimumWidth(300)
        
        self.estructura_score_label = QtWidgets.QLabel("Score: 0%")
        self.estructura_score_label.setFont(QtGui.QFont("Arial", 20, QtGui.QFont.Bold))
        self.estructura_score_label.setStyleSheet("color: #FFD700;")
        self.estructura_score_label.setAlignment(QtCore.Qt.AlignCenter)
        estructura_layout.addWidget(self.estructura_score_label)
        
        self.estructura_direction_label = QtWidgets.QLabel("Direccion: NEUTRAL")
        self.estructura_direction_label.setStyleSheet("color: #b0b0b0; font-size: 12px;")
        estructura_layout.addWidget(self.estructura_direction_label)
        
        estructura_details = QtWidgets.QLabel(
            "• BOS: Break of Structure\n"
            "• CHoCH: Change of Character\n"
            "• Order Blocks institucionales\n"
            "• FVG: Fair Value Gaps"
        )
        estructura_details.setStyleSheet("color: #808080; font-size: 10px;")
        estructura_layout.addWidget(estructura_details)
        
        self.estructura_signals_text = QtWidgets.QTextEdit()
        self.estructura_signals_text.setMaximumHeight(100)
        self.estructura_signals_text.setReadOnly(True)
        self.estructura_signals_text.setStyleSheet("background-color: #0f0f23; color: #FFD700; border: 1px solid #1a2a4c;")
        estructura_layout.addWidget(self.estructura_signals_text)
        
        strategies_layout.addWidget(estructura_frame)
        
        # === ESTRATEGIA 3: CONFLUENCE MTF ===
        confluence_frame, confluence_layout = self._create_titled_frame("🎯 ESTRATEGIA 3: Confluence MTF (30%)", tab_widget)
        confluence_frame.setMinimumWidth(300)
        
        self.confluence_score_label = QtWidgets.QLabel("Score: 0%")
        self.confluence_score_label.setFont(QtGui.QFont("Arial", 20, QtGui.QFont.Bold))
        self.confluence_score_label.setStyleSheet("color: #FF6B6B;")
        self.confluence_score_label.setAlignment(QtCore.Qt.AlignCenter)
        confluence_layout.addWidget(self.confluence_score_label)
        
        self.confluence_direction_label = QtWidgets.QLabel("Direccion: NEUTRAL")
        self.confluence_direction_label.setStyleSheet("color: #b0b0b0; font-size: 12px;")
        confluence_layout.addWidget(self.confluence_direction_label)
        
        confluence_details = QtWidgets.QLabel(
            "• Multi-Timeframe: 5m/15m/30m/1h\n"
            "• Fibonacci retrocesos/extensiones\n"
            "• Patrones armonicos (AB=CD)\n"
            "• Correlacion entre pares"
        )
        confluence_details.setStyleSheet("color: #808080; font-size: 10px;")
        confluence_layout.addWidget(confluence_details)
        
        self.confluence_signals_text = QtWidgets.QTextEdit()
        self.confluence_signals_text.setMaximumHeight(100)
        self.confluence_signals_text.setReadOnly(True)
        self.confluence_signals_text.setStyleSheet("background-color: #0f0f23; color: #FF6B6B; border: 1px solid #1a2a4c;")
        confluence_layout.addWidget(self.confluence_signals_text)
        
        strategies_layout.addWidget(confluence_frame)
        
        tab_layout.addLayout(strategies_layout)
        
        # === RESULTADO COMBINADO ===
        combined_frame = QtWidgets.QFrame()
        combined_frame.setStyleSheet(
            "background-color: #0f0f23; border: 3px solid #00d4aa; border-radius: 15px; padding: 15px;"
        )
        combined_layout = QtWidgets.QHBoxLayout(combined_frame)
        
        # Score combinado
        combined_score_layout = QtWidgets.QVBoxLayout()
        self.triple_combined_score = QtWidgets.QLabel("0.0%")
        self.triple_combined_score.setFont(QtGui.QFont("Arial", 48, QtGui.QFont.Bold))
        self.triple_combined_score.setStyleSheet("color: #00d4aa;")
        self.triple_combined_score.setAlignment(QtCore.Qt.AlignCenter)
        combined_score_layout.addWidget(self.triple_combined_score)
        
        score_label = QtWidgets.QLabel("SCORE COMBINADO")
        score_label.setStyleSheet("color: #b0b0b0;")
        score_label.setAlignment(QtCore.Qt.AlignCenter)
        combined_score_layout.addWidget(score_label)
        combined_layout.addLayout(combined_score_layout)
        
        # Direccion
        direction_layout = QtWidgets.QVBoxLayout()
        self.triple_direction_label = QtWidgets.QLabel("NEUTRAL")
        self.triple_direction_label.setFont(QtGui.QFont("Arial", 24, QtGui.QFont.Bold))
        self.triple_direction_label.setStyleSheet("color: #b0b0b0;")
        self.triple_direction_label.setAlignment(QtCore.Qt.AlignCenter)
        direction_layout.addWidget(self.triple_direction_label)
        
        direction_title = QtWidgets.QLabel("DIRECCION")
        direction_title.setStyleSheet("color: #b0b0b0;")
        direction_title.setAlignment(QtCore.Qt.AlignCenter)
        direction_layout.addWidget(direction_title)
        combined_layout.addLayout(direction_layout)
        
        # Estrategias alineadas
        aligned_layout = QtWidgets.QVBoxLayout()
        self.triple_aligned_label = QtWidgets.QLabel("0/3")
        self.triple_aligned_label.setFont(QtGui.QFont("Arial", 24, QtGui.QFont.Bold))
        self.triple_aligned_label.setStyleSheet("color: #FF6B6B;")
        self.triple_aligned_label.setAlignment(QtCore.Qt.AlignCenter)
        aligned_layout.addWidget(self.triple_aligned_label)
        
        aligned_title = QtWidgets.QLabel("ESTRATEGIAS ALINEADAS")
        aligned_title.setStyleSheet("color: #b0b0b0;")
        aligned_title.setAlignment(QtCore.Qt.AlignCenter)
        aligned_layout.addWidget(aligned_title)
        combined_layout.addLayout(aligned_layout)
        
        # Estado
        status_layout = QtWidgets.QVBoxLayout()
        self.triple_status_label = QtWidgets.QLabel("ESPERANDO")
        self.triple_status_label.setFont(QtGui.QFont("Arial", 18, QtGui.QFont.Bold))
        self.triple_status_label.setStyleSheet("color: #b0b0b0; background-color: #1a2a4c; padding: 10px; border-radius: 8px;")
        self.triple_status_label.setAlignment(QtCore.Qt.AlignCenter)
        status_layout.addWidget(self.triple_status_label)
        
        status_title = QtWidgets.QLabel("ESTADO SENAL")
        status_title.setStyleSheet("color: #b0b0b0;")
        status_title.setAlignment(QtCore.Qt.AlignCenter)
        status_layout.addWidget(status_title)
        combined_layout.addLayout(status_layout)
        
        tab_layout.addWidget(combined_frame)
        
        # === SEÑALES DETECTADAS ===
        signals_frame, signals_layout = self._create_titled_frame("📋 Senales de las 3 Estrategias", tab_widget)
        self.triple_signals_list = QtWidgets.QTextEdit()
        self.triple_signals_list.setMaximumHeight(150)
        self.triple_signals_list.setReadOnly(True)
        self.triple_signals_list.setStyleSheet("background-color: #0f0f23; color: #00d4aa; border: 1px solid #1a2a4c;")
        signals_layout.addWidget(self.triple_signals_list)
        tab_layout.addWidget(signals_frame)
        
        return tab_widget
    
    def update_triple_strategy_tab(self, triple_result: dict):
        """Actualiza la pestaña de Triple Estrategia con los resultados"""
        try:
            if not triple_result:
                return
            
            # Estrategia 1: Momentum
            momentum = triple_result.get('momentum', {})
            self.momentum_score_label.setText(f"Score: {momentum.get('score', 0):.0f}%")
            self.momentum_direction_label.setText(f"Direccion: {momentum.get('direction', 'NEUTRAL')}")
            signals_m = '\n'.join(momentum.get('signals', []))
            self.momentum_signals_text.setPlainText(signals_m if signals_m else "Sin senales detectadas")
            
            # Color segun score
            m_score = momentum.get('score', 0)
            if m_score >= 80:
                self.momentum_score_label.setStyleSheet("color: #00FF00; font-size: 20px; font-weight: bold;")
            elif m_score >= 60:
                self.momentum_score_label.setStyleSheet("color: #FFD700; font-size: 20px; font-weight: bold;")
            else:
                self.momentum_score_label.setStyleSheet("color: #FF6B6B; font-size: 20px; font-weight: bold;")
            
            # Estrategia 2: Estructura
            estructura = triple_result.get('estructura', {})
            self.estructura_score_label.setText(f"Score: {estructura.get('score', 0):.0f}%")
            self.estructura_direction_label.setText(f"Direccion: {estructura.get('direction', 'NEUTRAL')}")
            signals_e = '\n'.join(estructura.get('signals', []))
            self.estructura_signals_text.setPlainText(signals_e if signals_e else "Sin senales detectadas")
            
            e_score = estructura.get('score', 0)
            if e_score >= 80:
                self.estructura_score_label.setStyleSheet("color: #00FF00; font-size: 20px; font-weight: bold;")
            elif e_score >= 60:
                self.estructura_score_label.setStyleSheet("color: #FFD700; font-size: 20px; font-weight: bold;")
            else:
                self.estructura_score_label.setStyleSheet("color: #FF6B6B; font-size: 20px; font-weight: bold;")
            
            # Estrategia 3: Confluence
            confluence = triple_result.get('confluence', {})
            self.confluence_score_label.setText(f"Score: {confluence.get('score', 0):.0f}%")
            self.confluence_direction_label.setText(f"Direccion: {confluence.get('direction', 'NEUTRAL')}")
            signals_c = '\n'.join(confluence.get('signals', []))
            self.confluence_signals_text.setPlainText(signals_c if signals_c else "Sin senales detectadas")
            
            c_score = confluence.get('score', 0)
            if c_score >= 80:
                self.confluence_score_label.setStyleSheet("color: #00FF00; font-size: 20px; font-weight: bold;")
            elif c_score >= 60:
                self.confluence_score_label.setStyleSheet("color: #FFD700; font-size: 20px; font-weight: bold;")
            else:
                self.confluence_score_label.setStyleSheet("color: #FF6B6B; font-size: 20px; font-weight: bold;")
            
            # Resultado combinado
            combined = triple_result.get('combined', {})
            combined_score = combined.get('combined_score', 0)
            self.triple_combined_score.setText(f"{combined_score:.1f}%")
            
            direction = combined.get('direction', 'NEUTRAL')
            self.triple_direction_label.setText(direction)
            
            aligned = combined.get('strategies_aligned', 0)
            self.triple_aligned_label.setText(f"{aligned}/3")
            
            # Colores segun resultado
            if combined_score >= 85 and aligned >= 3:
                self.triple_combined_score.setStyleSheet("color: #00FF00; font-size: 48px; font-weight: bold;")
                self.triple_status_label.setText("✅ SENAL VALIDA")
                self.triple_status_label.setStyleSheet("color: #00FF00; background-color: #1a4c2a; padding: 10px; border-radius: 8px; font-size: 18px; font-weight: bold;")
                self.triple_aligned_label.setStyleSheet("color: #00FF00; font-size: 24px; font-weight: bold;")
            elif combined_score >= 70:
                self.triple_combined_score.setStyleSheet("color: #FFD700; font-size: 48px; font-weight: bold;")
                self.triple_status_label.setText("⚠️ CASI VALIDA")
                self.triple_status_label.setStyleSheet("color: #FFD700; background-color: #4c4c1a; padding: 10px; border-radius: 8px; font-size: 18px; font-weight: bold;")
                self.triple_aligned_label.setStyleSheet("color: #FFD700; font-size: 24px; font-weight: bold;")
            else:
                self.triple_combined_score.setStyleSheet("color: #FF6B6B; font-size: 48px; font-weight: bold;")
                self.triple_status_label.setText("❌ NO VALIDA")
                self.triple_status_label.setStyleSheet("color: #FF6B6B; background-color: #4c1a1a; padding: 10px; border-radius: 8px; font-size: 18px; font-weight: bold;")
                self.triple_aligned_label.setStyleSheet("color: #FF6B6B; font-size: 24px; font-weight: bold;")
            
            # Color direccion
            if direction == 'BULLISH':
                self.triple_direction_label.setStyleSheet("color: #00FF00; font-size: 24px; font-weight: bold;")
            elif direction == 'BEARISH':
                self.triple_direction_label.setStyleSheet("color: #FF6B6B; font-size: 24px; font-weight: bold;")
            else:
                self.triple_direction_label.setStyleSheet("color: #b0b0b0; font-size: 24px; font-weight: bold;")
            
            # Todas las señales
            all_signals = combined.get('signals', [])
            self.triple_signals_list.setPlainText('\n'.join(all_signals) if all_signals else "Sin senales combinadas")
            
        except Exception as e:
            logger.error(f"Error actualizando tab Triple Estrategia: {e}")
    ...
    def _create_optimized_neural_tab(self):
        ...
        training_frame, training_layout = self._create_titled_frame("🧠 Red Neuronal Avanzada v36.0.0.0 - OPTIMIZADA", tab_widget)
        button_layout = QtWidgets.QHBoxLayout()
    ...
    def start_bot_optimized(self):
        ...
        self.status_label.setText("🚀 Triple Estrategia ACTIVA")
        self.status_label.setStyleSheet("color: #00d4aa; font-weight: bold;")
        self.start_btn.setEnabled(False)
        self.stop_btn.setEnabled(True)
        self.bot.start_optimized()
        logger.info("🚀 Bot v36.0.0.0 Triple Estrategia iniciado")
        self.log_message("🚀 Bot v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE ACTIVADO")
        self.log_message("📊 Analizando: Momentum + Estructura + Confluence MTF")
        self.log_message("🎯 Objetivo: 3/3 estrategias alineadas + Score >= 85%")
        self.update_pair_scan_progress(0)
        self.highlight_progress_bar.setVisible(False)
        self.highlight_progress_bar.setValue(0)
        self.highlight_progress_bar.setFormat("Esperando señal TRIPLE ESTRATEGIA...")
    ...
    def _train_neural_thread_optimized(self):
        ...
        self.log_message("🧠 Iniciando entrenamiento IA optimizada v36.0.0.0...")
        def progress_callback(value):
            QMetaObject.invokeMethod(
                self.progress_bar, "setValue", 
                QtCore.Qt.QueuedConnection, 
                ...
                if msg_type == 'log_message':
                    ...
                    def _update_confirmed(progress_data):
                        try:
                            ...
                            # Determinar hito actual usando valores dinámicos
...
def main_backend():
    ...
    print("="*80)
    print("🚀 CRYPTO BOT PRO v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE - MODO BACKEND")
...
def main():
    ...
    try:
        print("="*70)
        print("🚀 CRYPTO BOT PRO v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE")
        print("📊 Estrategia 1: Momentum Institucional (35%)")
        print("📈 Estrategia 2: Estructura de Mercado (35%)")
        print("🎯 Estrategia 3: Confluence MTF (30%)")
        print("👨‍💻 Desarrollado por Lic: Adolfo Daniel Aguirre")
        ...
        app.setApplicationName("Crypto Bot Pro v36.0.0.0")
        app.setApplicationVersion("v36.0.0.0")

        # ✅ CREAR CONFIGURACIÓN Y GUI
        config_optimized = AdvancedTradingConfig()
        main_window = OptimizedCryptoBotGUI(config_optimized)

        logger.info("🚀 CRYPTO BOT PRO v36.0.0.0 - TRIPLE ESTRATEGIA 85% WIN RATE - INICIADO EXITOSAMENTE")
...
class ModeSelectionDialog(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        title = QtWidgets.QLabel("Crypto Bot Pro v36.0.0.0")
        title.setStyleSheet("""
            font-size: 28px; 
            font-weight: bold; 
            color: #00d4aa;
...
def check_production_readiness():
    ...
    if not all_ok:
        logger.warning("⚠️ Algunas verificaciones de entorno fallaron. El bot puede operar pero revise los logs.")
    return all_ok
