#!/usr/bin/env python3
"""
Crypto Bot Pro v34.0.0.1 - Sistema Avanzado de Trading con IA (Versión Escritorio Optimizada - CORREGIDA)
Aplicación de escritorio usando PyQt5
Estrategia Unificada: EMA_TDI_PRICE_ACTION_NEURAL
Desarrollado por Lic. Adolfo Daniel Aguirre
✅ CORRECCIÓN FINAL v34.0.0.1:
   - ✅ Señal eliminada de GUI al cerrar en Telegram
   - ✅ Limpieza total de cachés tras cierre
   - ✅ Solo 1 señal activa a la vez
   - ✅ Barra de progreso para DESTACADA (5 min)
   - ✅ Foto SOLO para CONFIRMADA (DESTACADA sin foto)
   - ✅ Promoción automática DESTACADA → CONFIRMADA
   - ✅ Cierre automático al 1.0 % de profit
ULTIMA MODIFICACION: 12/11/2025 - FULL OPERATIVO
"""
...
# ✅ Permitir ejecución en Replit con VNC
print("✅ Iniciando Crypto Bot Pro v34.0.0.1 ")
...
class AdvancedTradingConfig:
    ...
    def __init__(self):
        ...
        # Umbrales de señales (v32.0.22.17 - Sincronizados con JSON para generar señales)
        self.MIN_NEURAL_DESTACADA = 50.0      # IA mínima para DESTACADA
        self.MIN_TECHNICAL_DESTACADA = 40.0   # Técnico mínimo para DESTACADA
        self.MIN_ALIGNMENT_DESTACADA = 33.0   # Alineación mínima para DESTACADA
        self.MIN_NEURAL_CONFIRMADA = 60.0     # IA mínima para promoción a CONFIRMADA
        self.MIN_TECHNICAL_CONFIRMADA = 50.0  # Técnico mínimo para CONFIRMADA
        self.MIN_ALIGNMENT_CONFIRMADA = 50.0  # Alineación mínima para CONFIRMADA
        self.MIN_NEURAL_VALIDATION = 85.0
        self.REQUIRE_CANDLE_PATTERN = True
        self.REQUIRE_ENTRY_SETUP = True
        self.ENTRY_EMA_PERIOD = 21
        self.MIN_ENTRY_PATTERN_CONFIDENCE = 60.0
        self.MAX_ENTRY_DISTANCE_ATR = 1.2
        self.MAX_ENTRY_CANDLE_RANGE_ATR = 1.6
        self.MAX_ENTRY_CANDLE_BODY_ATR = 1.1
        self.ENTRY_PULLBACK_REQUIRED = True
        self.ENTRY_CONFLUENCE_BYPASS = 75
        ...
        self.TAKE_PROFIT_PERCENT = 3.0     # Piso mínimo TP (%) para baja volatilidad
        self.MIN_RISK_REWARD_RATIO = 1.0
        ...
        self.auto_trading_enabled = False  # Alias en minúsculas para compatibilidad
        self.AUTO_TRADE_QUANTITY_USDT = 10
        self.TRAILING_STOP_ENABLED = True
        self.TRAILING_STOP_DISTANCE = 0.3
...
class OptimizedTechnicalAnalyzer:
    ...
    def calculate_atr(self, df: pd.DataFrame, period: int = 14) -> float:
        """
        Calcula el Average True Range (ATR) para gestión dinámica de riesgo.
        """
        try:
            if df is None or len(df) < period + 1:
                return 0.0
    ...
    def calculate_dynamic_stop_loss(self, entry_price: float, atr: float, 
        ...
        Incluye pisos mínimos para mercados de baja volatilidad.
        """
        try:
            # Pisos mínimos basados en porcentaje (configurables)
            ...
            if atr <= 0 or entry_price <= 0:
                ...
                # ATR con pisos mínimos (evita stops muy ajustados)
            ...
            logger.debug(f"Error calculando SL/TP dinámico: {e}")
            return {
    ...
    def calculate_pattern_confluence_score(self, df: pd.DataFrame, trend_direction: 'TrendDirection') -> dict:
        """
        Calcula un score de confluencia combinando:
        - Patrón de velas (40%)
        - Soporte/Resistencia (25%)
        - Volumen (20%)
        - Alineación con tendencia (15%)
        ...
        try:
            ...
            if pattern_type != 'NEUTRAL' and pattern_conf >= 60:
                if (is_bullish_trend and pattern_type == 'BULLISH') or (is_bearish_trend and pattern_type == 'BEARISH'):
                    trend_aligned = True
                    pattern_score = min(40, pattern_conf * 0.4)
                    details.append(f"Patron {pattern_name} ({pattern_conf}%) alineado con tendencia")
                elif pattern_type != 'NEUTRAL':
                    pattern_score = min(25, pattern_conf * 0.25)
                    details.append(f"Patron {pattern_name} ({pattern_conf}%) contra tendencia")
            elif pattern_conf > 0:
                pattern_score = min(15, pattern_conf * 0.15)
                details.append(f"Patron debil: {pattern_name} ({pattern_conf}%)")
            ...
            if is_bullish_trend and sr_data.get('closest_support'):
                ...
                if 0 <= dist <= 0.02:
                    sr_score = min(25, sup['strength'] * 0.25)
                    sr_level = {'type': 'SUPPORT', 'price': sup['price'], 'strength': sup['strength'], 'distance': dist}
                    details.append(f"Cerca de soporte ({dist:.1%}, fuerza {sup['strength']:.0f}%)")
                elif 0 <= dist <= 0.04:
                ...
                if 0 <= dist <= 0.02:
                    sr_score = min(25, res['strength'] * 0.25)
                    sr_level = {'type': 'RESISTANCE', 'price': res['price'], 'strength': res['strength'], 'distance': dist}
                    details.append(f"Cerca de resistencia ({dist:.1%}, fuerza {res['strength']:.0f}%)")
                elif 0 <= dist <= 0.04:
            ...
            if vol_confirmed:
                vol_score = 20
                details.append("Volumen alto confirmado")
            else:
                try:
                    vol_analysis = self.calculate_volume_confidence(df, 20)
                    if vol_analysis.get('valid', False):
                        vol_ratio = vol_analysis.get('ratio', 0)
                        if vol_ratio > 1.5:
                            vol_score = 15
                            details.append(f"Volumen elevado ({vol_ratio:.1f}x)")
                        elif vol_ratio > 1.2:
                            vol_score = 10
            ...
            if trend_aligned:
                trend_score = 15
                details.append("Alineado con tendencia principal")
            elif pattern_type != 'NEUTRAL':
                trend_score = 5
            ...
            if pattern_name in high_conf_patterns and trend_aligned and vol_confirmed:
                bonus = 10
                details.append("BONUS: Patron fuerte + Vol + Tendencia (+10)")
            elif pattern_name in high_conf_patterns and sr_level and sr_level.get('strength', 0) >= 60:
                bonus = 8
                details.append("BONUS: Patron fuerte + S/R fuerte (+8)")
            elif trend_aligned and vol_confirmed and sr_level:
                bonus = 5
                details.append("BONUS: Triple confluencia (+5)")
    ...
    def validate_ultra_selective_conditions(self, symbol: str,
        ...
        """
        ✅ VALIDACIÓN SIMPLIFICADA v34.0.0.1 – SOLO 4 CRITERIOS CLAVE
    ...
    def _build_optimized_model(self, input_size):
        class OptimizedNet(nn.Module):
            def __init__(self, input_size, hidden_layers, dropout):
                ...
                layers = []
                prev_size = input_size
                layers.append(nn.Linear(prev_size, hidden_layers[0]))
                ...
                for i, size in enumerate(hidden_layers[1:], 1):
                    ...
                    layers.append(nn.ReLU())
                    layers.append(nn.Dropout(dropout * 0.8))
                    prev_size = size
        ...
        return OptimizedNet(input_size, self.config.NEURAL_HIDDEN_LAYERS, self.config.NEURAL_DROPOUT)
...
# ============================================================================
# SISTEMA DE PROCESAMIENTO DE SEÑALES OPTIMIZADO (CORREGIDO v34.0.0.1)
# ============================================================================
class OptimizedSignalProcessor:
    ...
    # ------------------------------------------------------------------
    # 5. SEGUIMIENTO POR ETAPAS + NOTIFICACIONES TELEGRAM
    # ------------------------------------------------------------------
    def track_signal_stages(self, symbol: str, signal_hash: str,
                            entry_price: float, is_buy: bool,
                            current_price: float) -> dict:
        """
        Evalúa cada etapa (1 %, 2 %, 3 %, -1 %, 180 min, reversión)
        y envía notificación a Telegram si corresponde.
        ...
        # --- Etapas desde config (SOLO para CONFIRMADA) ------------------------------------------------------
        # ✅ CORRECCIÓN: Verificar estado de la señal antes de enviar milestones
...
# ========== CLIENTE TELEGRAM OPTIMIZADO (CORREGIDO v34.0.0.1) ==========
class OptimizedTelegramClient:
    """
    Cliente optimizado para Telegram con manejo robusto de errores,
    depuración detallada, rate limiting y cola de mensajes.
    """
    def __init__(self, config):
        self.config = config
        self.base_url = f"https://api.telegram.org/bot{config.telegram_bot_token}"

        # Configurar sesión de requests si está disponible
        self.session = None
    ...
    def _rate_limit_check(self):
        """Implementar rate limiting con espera activa"""
        current_time = time.time()
...
class RobustWebSocketManager:
    """
    Gestor WebSocket Mejorado con Mejor Manejo de Errores, Lógica de Reconexión y Fallback a API REST
    """
    def __init__(self, symbols: List[str], intervalo: str, callback: Callable, max_reconnect_attempts: int = 50):
        # ✅ MEJORA 1: Validar y limpiar símbolos inválidos
        self.symbols = [s.strip().upper() for s in symbols if s and isinstance(s, str)]
        if not self.symbols:
            logger.error("⚠️ Ningún símbolo válido proporcionado a RobustWebSocketManager")
            self.symbols = []
        ...
        self.base_retraso_reconexion = 2  # Segundos base para reconexión
        self.max_retraso_reconexion = 30  # ✅ MEJORA 2: Reducido a 30s max (más agresivo)

        # ✅ URL WebSocket para Binance Futures (PERPETUALS)
        # fstream.binance.com es el endpoint oficial para Futures
        ...
        self.ultimo_ping = 0
        # ✅ MEJORA ARGENTINA: ping_interval DEBE ser > ping_timeout (requisito websocket-client)
        # Aumentados para mayor estabilidad y tolerancia a latencia
        self.intervalo_ping = 90  # Intervalo entre pings (90s) - más tolerante
        self.ping_timeout = 60    # Tiempo de espera para pong (60s) - tolerante a alta latencia

        # Gestión del modo de respaldo (Fallback)
        ...
        self.ultimos_errores = []  # ✅ MEJORA 4: Rastrear últimos errores para diagnóstico

        # Control de hilo para reconexión
        self._stop_event = threading.Event()
    ...
    def iniciar(self):
        ...
        if not WEBSOCKET_AVAILABLE:
            logger.info("⚠️ WebSocket no disponible - Usando modo polling/REST")
            self._start_fallback()
            return

        if self.ejecutando:
            logger.warning("WebSocket ya está en ejecución")
            return
        ...
        self.thread = threading.Thread(target=self._gestor_conexion, daemon=True)
        self.thread.start()
        logger.info(f"Gestor WebSocket iniciado para símbolos: {self.symbols}")
...
class SignalTracker:
    ...
    def cancel_signal(self, signal_hash: str, reason: str = "Cancelada por sistema"):
        with self.lock:
            ...
            logger.info(f"❌ Señal {symbol} ({signal_hash[:8]}) cancelada: {reason}")
...
# ============================================================================
# BOT PRINCIPAL OPTIMIZADO (CORREGIDO v34.0.0.1)
# ============================================================================
class OptimizedTradingBot:
    ...
    def _on_signal_closed_callback(self, symbol: str, report: dict):
        """Callback desde SignalTracker para limpiar GUI y estado al cerrar una señal"""
        reason = report.get('reason', 'UNKNOWN')
        ...
        # ✅ CRÍTICO: Limpiar panel de señal activa en GUI y OCULTAR barra
        self._safe_gui_queue_put(('update_highlight_progress', 0))  # Resetear barra de progreso
        self._safe_gui_queue_put(('hide_progress_bar', None))       # ✅ NUEVO: Ocultar barra completamente
        ...
        if reason == "HIGHLIGHT_TIMEOUT":
            ...
            return  # Salir (ya se envió la notificación específica)
    ...
    def start_optimized(self):
        """Iniciar bot con optimizaciones — CORREGIDO para emitir señales y mostrar progreso por ciclo"""
        try:
            self.running = True
            logger.info("🚀 Bot Optimizado v34.0.0.1 iniciando...")
            ...
            self._safe_gui_queue_put(('log_message', "🚀 Bot Avanzado Optimizado v34.0.0.1 iniciando sistema avanzado..."))
            ...
            # === 3. Iniciar monitoreo continuo de señales (ES CLAVE PARA TIMEOUT Y PROMOCIÓN) ===
            ...
            logger.info("📊 Sistema de monitoreo continuo de señales iniciado")
            ...
            self._safe_gui_queue_put(('log_message', "✅ Bot Optimizado v34.0.0.1 iniciado correctamente"))
    ...
    def _process_high_quality_signal(self, symbol: str, df_primary: pd.DataFrame, df_entry: pd.DataFrame, analysis_result: dict):
        ...
        # === 7. Registrar en GUI y métricas ===
        self.performance_metrics['total_signals'] += 1
        self.active_signals.append(signal_package)
        self._safe_gui_queue_put(('signal_found', None))
        self._safe_gui_queue_put(('log_message', 
            f"🚨 SEÑAL DESTACADA: {symbol} | IA={neural_score:.1f}% | Técnico={technical_pct:.1f}% | Alineación={alignment_pct:.1f}%"))
...
class SignalTracker:
    ...
    # 🔥 MÉTODO CRÍTICO: PROMOCIÓN AUTOMÁTICA
    def check_promotion_timer(self) -> List[str]:
        """
        Verifica señales DESTACADA para promoción o cancelación.
        Returns:
...
class OptimizedCryptoBotGUI(QtWidgets.QMainWindow):
    def __init__(self, config_instance):
        super().__init__()
        try:
            self.setWindowTitle("Crypto Bot Pro v34.0.0.1 - Propiedad Intelectual de Marketeradolfo")
            ...
            self._init_tabs()
            # Load initial data
            self.log_message("🤖 Bienvenido a Crypto Bot Pro v34.0.0.1 Desarrolado por Lic: Adolfo Daniel Aguirre")
    ...
    def _show_about_dialog(self):
        ...
        about_text = """
<h2 style='color: #00d4aa;'>Crypto Bot Pro v34.0.0.1</h2>
...
"""
    ...
    def _init_header(self):
        ...
        title_label = QtWidgets.QLabel("🚀 Crypto Bot Pro v34.0.0.1 - Desarrolado por Lic: Adolfo Daniel Aguirre")
    ...
    def _process_high_quality_signal(self, symbol: str, df_primary: pd.DataFrame, df_entry: pd.DataFrame, analysis_result: dict):
        ...
        self._safe_gui_queue_put((
            'log_message',
            f"🚨 SEÑAL DESTACADA: {symbol} | "
            f"IA={neural_score:.1f}% | Técnico={technical_pct:.1f}% | Alineación={alignment_pct:.1f}%"
        ))
    ...
    def _create_optimized_neural_tab(self):
        ...
        training_frame, training_layout = self._create_titled_frame("🧠 Red Neuronal Avanzada v34.0.0.1 - OPTIMIZADA", tab_widget)
        button_layout = QtWidgets.QHBoxLayout()
    ...
    def _create_optimized_config_tab(self):
        ...
        autotrader_frame, autotrader_layout = self._create_titled_frame("🤖 Configuración AutoTrader", scroll_widget)
    ...
    def start_bot_optimized(self):
        ...
        logger.info("🚀 Bot Optimizado v34.0.0.1 iniciado")
        self.log_message("🚀 Bot Avanzado Optimizado v34.0.0.1 iniciado con mejoras de rendimiento")
    ...
    def _train_neural_thread_optimized(self):
        ...
        self.log_message("🧠 Iniciando entrenamiento IA optimizada v34.0.0.1...")
        def progress_callback(value):
            QMetaObject.invokeMethod(
                self.progress_bar, "setValue", 
                QtCore.Qt.QueuedConnection, 
        ...
        while messages_processed < max_messages_per_batch:
            try:
                ...
                if msg_type == 'log_message':
                    ...
                    # ✅ Limpiar panel principal de señal activa cuando expira DESTACADA
                    def _clear_signal_display():
                        try:
                            ...
                            if hasattr(self, 'details_text'):
                                self.details_text.setText("Esperando nueva señal...")
                            logger.info("🧹 Panel de señal activa limpiado")
                        except Exception as e:
                            logger.error(f"[ERROR] Error limpiando panel de señal: {e}")
                    QtCore.QTimer.singleShot(0, _clear_signal_display)

                elif msg_type == 'hide_progress_bar':
                    # ✅ NUEVO: Ocultar barra de progreso completamente tras cierre
                    def _hide_bar():
                        try:
                            self.highlight_progress_bar.setVisible(False)
                            self.highlight_progress_bar.setValue(0)
                            self.highlight_progress_bar.setFormat("Esperando señal...")
                            ...
                            logger.error(f"Error ocultando barra: {e}")
...
def main_backend():
    ...
    print("="*80)
    print("🚀 CRYPTO BOT PRO v34.0.0.1 - MODO BACKEND (SIN GUI)")
...
def main():
    ...
    try:
        print("="*70)
        print("🚀 CRYPTO BOT PRO v34.0.0.1 - Desarrollado por Lic: Adolfo Daniel Aguirre")
        ...
        app.setApplicationName("Crypto Bot Pro v34.0.0.1")
        app.setApplicationVersion("v34.0.0.1")

        # ✅ CREAR CONFIGURACIÓN Y GUI
        config_optimized = AdvancedTradingConfig()
        main_window = OptimizedCryptoBotGUI(config_optimized)

        logger.info("🚀 CRYPTO BOT PRO v34.0.0.1 OPTIMIZADO INICIADO EXITOSAMENTE")
...
class ModeSelectionDialog(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        title = QtWidgets.QLabel("Crypto Bot Pro v34.0.0.1")
        title.setStyleSheet("""
            font-size: 28px; 
            font-weight: bold; 
            color: #00d4aa;
